...
// Include messages used in montreal topology
...
public:
    ...
    virtual ~Montreal()
    {
        DESTRUCTOR_PUB(amazon)
        DESTRUCTOR_PUB(danube)
        DESTRUCTOR_PUB(ganges)
        DESTRUCTOR_PUB(nile)
    ...
    void run(int experiment_duration_sec)
    {
        ...
        std::thread t_pub_amazon   (&Montreal::publish<Stamped9Float32>,  this, writer_amazon_,   10,  36,    "amazon");
        std::thread t_pub_danube   (&Montreal::publish<StampedInt64>,     this, writer_danube_,   10,  8,     "danube");
        std::thread t_pub_ganges   (&Montreal::publish<Stamped4Int32>,    this, writer_ganges_,   10,  16,    "ganges");
        std::thread t_pub_nile     (&Montreal::publish<Stamped4Int32>,    this, writer_nile_,     10,  16,    "nile");
...
int main(int argc, char** argv)
{
    ...
    std::cout << "Starting montreal. Duration: " << experiment_duration_sec
              << " seconds.\n" << std::endl;
    ...
    if(montreal->init()) {
        ...
        montreal->run(experiment_duration_sec);
    } else {
