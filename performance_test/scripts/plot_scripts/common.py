...
def organize_data(data_samples, x_key, separator, uncountable_data):
    ...
    e.g.    separator = "msg_type", x_key = "subs"
...
def get_plot_data(data, x_key, y_key):
    ...
    switcher = {
        ...
        'msg_type' : lambda data: [ convert_to_size(values['msg_type'])/1024   for _, values in sorted(data.items()) ],
        ...
        'latency': lambda data: [ values['latency']/values['valid_subscribers']   for _, values in sorted(data.items()) ],
        # cpu and RAM
        'time' : lambda data: [ values['time']   for _, values in sorted(data.items()) ],
        'cpu'  : lambda data: [ values['cpu']/values['num_experiments']   for _, values in sorted(data.items()) ],
        'rss'  : lambda data: [ (values['rss']/ 1000)/values['num_experiments']   for _, values in sorted(data.items()) ],
        'vsz'  : lambda data: [ (values['vsz']/ 1000)/values['num_experiments']   for _, values in sorted(data.items()) ]
    }

    x = switcher[x_key](data)
    y = switcher[y_key](data)
...
def plot_function(data_dict, x_key, y1_keys, y2_keys, separator):
    ...
    for label, data in sorted(data_dict.items()):
        ...
        for y1_key in y1_keys:
            ...
            x, y = get_plot_data(data, x_key, y1_key)
        ...
        for y2_key in y2_keys:
            ...
            # extract the values from the data dict, according to the keys
            x, y = get_plot_data(data, x_key, y2_key)
