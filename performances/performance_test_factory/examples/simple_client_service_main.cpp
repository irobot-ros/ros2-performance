...
int main(int argc, char ** argv)
{
    ...
    performance_test::TemplateFactory<performance_test::Node> factory(use_ipc, use_ros_params, verbose, ros_namespace);
    performance_test::System<performance_test::Node> ros2_system(static_cast<performance_test::ExecutorType>(executor));
    ...
    std::vector<std::shared_ptr<performance_test::Node>> client_nodes =
        factory.create_periodic_client_nodes(0,
            n_clients,
            n_services,
            frequency,
            msg_type,
    ...
    ros2_system.add_node(client_nodes);
    ...
    // TODO implement the check discovery API also for clients/services
