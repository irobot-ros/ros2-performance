    ...
    do
      ...
      do
        ...
        do
          ...
          SUB_CMD_ARGS="--subs $NUM_SUBSCRIBERS --pubs $NUM_PUBLISHERS --msg_type $MSG_TYPE --frequency -1 --duration $DURATION --filename $LAT_REL_FILE_PATH"
          PUB_CMD_ARGS="--subs $NUM_SUBSCRIBERS --pubs $NUM_PUBLISHERS --msg_type $MSG_TYPE --msg_size $MSG_SIZE --frequency $PUB_FREQ --duration $DURATION"
          ...
          if ! $MON_CPU_RAM ; then
              ...
              bash $UTILITIES_DIR/run_and_monitor.sh $CPU_RAM_FILE_PATH $SUB_CMD $SUB_CMD_ARGS &
              # give some time to the subscribers to be created before running the publisher
              sleep 2
              bash $UTILITIES_DIR/run.sh $PUB_CMD $PUB_CMD_ARGS &
          fi
          wait
