...
def parse_csv(file_path):
    ...
    data = {
        ...
        'msg_type' : '',            # type of exchanged messages
    ...
    # TODO: some values (spin and send frequencies, msg_type, duration) should be equal among all lines. The check has been removed now
    ...
    for row in reader:
        ...
        spin_frequency = int(row['spin_frequency'])
        avg_latency = float(row['avg_latency'])
        send_frequency = int(row['send_frequency'])
        msg_type = str(row['msg_type'])
        msg_size = int(row['msg_size'])
        experiment_duration = int(row['duration'])

        data['msg_type'] = msg_type
        data['msg_size'] = msg_size
        data['duration'] = experiment_duration
        pubs_ids.add(communication_id)

        if node_id == communication_id:
...
def main(argv):
    ...
    parser.add_argument('--x', type=str, required=True, choices=['pubs', 'subs', 'msg_type', 'msg_size'], help='value to display on x axis')
    ...
    parser.add_argument('--separator', nargs='+', default=[], choices=['spin_frequency', 'send_frequency', 'msg_type', 'msg_size', 'directory', 'duration', 'pubs', 'subs'], help='if not set all data are aggregated together, else aggregates only data which have the same value for the separator keys')
    ...
    __UNCOUNTABLE_DATA__ = ['directory', 'pubs', 'subs', 'spin_frequency', 'send_frequency', 'msg_type', 'msg_size', 'separator']
